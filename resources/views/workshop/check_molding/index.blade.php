...
@section('content')
    <section class="content" style="padding: 10px">
        ...
        <div class="row">
            ...
            <div class="col-md-3">
                <span style="font-weight: bold;">Status Cek</span>
                <div class="form-group">
                    <select class="form-control select2" style="width: 100%;" multiple="multiple" id='status'
        ...
        <div class="row">
            ...
            <div class="col-md-12">
                ...
                <h3 style="text-align: center; margin-bottom: 0px; background-color: #9e83a7; padding : 7px 0 7px 0;">
                    OUTSTANDING TEMUAN
                    <button class="btn btn-info pull-right" onclick="filter_data('','')"
                        style="padding: 2px 12px 2px 12px; margin-right: 7px; color: white"><i class="fa fa-refresh"></i>
                        refresh</button>
                </h3>
                    ...
                    <div class="row">
                        <div class="col-md-6">
                            ...
                            <div class="form-group">
                                ...
                                <div class="col-md-5">
                                    <select class="select3" id="status_problem" style="width: 100%"
                                        data-placeholder="Pilih Status">
                                        <option value=""></option>
                                        <option value="Open">Open</option>
                                        <option value="Temporary Close">Temporary Close</option>
                        ...
                        <div class="col-md-6">
                            <div class="form-group">
                                <label class="col-md-5 control-label">Perbaikan Foto <span
                        ...
                        <div class="col-md-12">
                            <div class="form-group">
                                <button class="btn btn-success" onclick="simpanPenanganan()"><i class="fa fa-check"></i>
                                    Simpan Penanganan</button>
                            </div>
                            <hr>
                        </div>
    ...
    <div class="modal modal-default fade" id="modal_schedule">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <h4 class="modal-title">
                        <center>Buat Schedule Check</center>
                    </h4>
                </div>
                <div class="modal-body">
                    <div class="row">
                        ...
                        <div class="col-xs-9">
                            <input type="month" id="periode_cek" class="form-control">
                        </div>
                    </div>
                    <div class="row">
                        <div class="col-xs-3">
                        ...
                        <div class="col-xs-9">
                            <button class="btn btn-sx bg-purple" onclick="add_schedule()"><i
                    ...
                    <div class="row">
                        <div class="col-xs-12">
                            <br>
                            <button class="btn btn-success pull-right" onclick="saveSchedule()"><i
...
@section('scripts')
    ...
    <script>
            ...
            $.get('{{ url('fetch/workshop/check_molding_vendor/monitoring') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $.each(result.data_all, function(key, value) {
                        if (status == '') {
                            if (value.status == "Open" || value.status == "Temporary Close" || value
                                .status == "In-Progress") {
                                body += '<tr>';
                                body += '<td>' + (key + 1) + '</td>';
                                ...
                                body += "<td>";
                                body +=
                                    "<button class='btn btn-primary'><i class='fa fa-info'></i> Details</button><br>";

                                if (value.status == "Open" || value.status == "Temporary Close" || value
                                    .status == "In-Progress") {
                                    body +=
                                        "<button class='btn btn-success' style='margin-top : 3px' onclick='penangananModal(" +
                                        value.id + ")'><i class='fa fa-edit'></i> Penanganan</button>";
                                }
                                body += "</td>";
                                body += '</tr>';
                            }
            ...
            $.each(data_all, function(key, value) {
                if (tanggal != '' && stat != '') {
                    if (value.status == stat && value.check_date == tanggal) {
                        ...
                        if (value.status == "Open" || value.status == "Temporary Close" || value.status ==
                            "In-Progress") {
                            body +=
                                "<button class='btn btn-success' style='margin-top : 3px' onclick='penangananModal(" +
                                value.id + ")'><i class='fa fa-edit'></i> Penanganan</button>";
        ...
        function penangananModal(ids) {
            ...
            $.get('{{ url('fetch/workshop/check_molding_vendor/penanganan/log') }}', data, function(result, status, xhr) {
                $.each(result.datas, function(key, value) {
                    ...
                    body +=
                        '<td><img style="max-width:200px" src="{{ url('workshop/Audit_Molding/Check_Molding/handling_att') }}/' +
                        value.handling_att1 +
                        '">&nbsp;<img style="max-width:200px" src="{{ url('workshop/Audit_Molding/Check_Molding/handling_att') }}/' +
                        value.handling_att2 + '"></td>';
                    body += '</tr>';
                })
        ...
        function add_schedule() {
            ...
            body += "<tr>";
            body += "<td><select class='select3 sch' style='width: 100%' data-placeholder='Pilih Molding'>";
            body += "<option value=''></option>";
			$.each(moldings, function(key, value) {
				body += "<option value='"+value.id+"'>"+value.molding_name+" #"+value.mold_number+"</option>";
			})
            body += "</select></td>";
            body += "<td><button class='btn btn-danger'><i class='fa fa-trash'></i></td>";
            body += "</tr>";
		...
		function saveSchedule() {
			...
			var id_molding = [];
			$('.sch').each(function(key, value) {
				id_molding.push($(value).val());
			});
