        ...
        <div class="wrap-contact100 col-xs-12 col-md-8 col-md-offset-2" style="padding: 0 10px; padding-bottom: 6%;">
            ...
            <div id="not_filled" style="width: 100%;">
                <div class="col-xs-12 col-md-8 col-md-offset-2" id="po_field" style="margin-left: 16.67%;">
                    ...
                    <input type="text" class="form-control" id="po_number" name="po_number"
                        placeholder="Please enter a 10-digit PO Number (e.g., 1000012345)" onkeyup="searhPo()">
                </div>


                <div class="col-xs-12 col-md-12" id="main"
                    style="display: none; vertical-align: middle; margin-top: 3%;">
                    <p><sup style="font-weight: bold; font-size: 10px;">*)</sup>&nbsp;Please tick the check column to
                        confirm that the PO has been received</p>
                    ...
                    <center>
                        <div class="container-contact100-form-btn" style="margin-top: 2%;">
                            <button class="contact1002-form-btn" type="button" onclick="clearAll()"
            ...
            <div id="already_filled" style="width: 100%; display: none; margin-top: 5%;">
                <div class="col-xs-12 col-md-12">
                    <center style="font-size: 20px">Thank you <span style="color: #3c8dbc;"
                            id="vendor_name"></span><br>
        ...
        if (po_number.length >= 7) {
            ...
            $.get('{{ url('fetch/po') }}', data, function(result, status, xhr) {
                if (result.status) {
                    ...
                    $('#vendor_name').text(result.vendor_name);
                    ...
                    for (var i = 0; i < result.data.length; i++) {
                        ...
                        tableData += result.data[i].material_number + '</td>';

                        tableData += '<td style="width:30%; padding:0px 5px 0px 5px; text-align:left;">';
                        tableData += result.data[i].material_description + '</td>';

                        tableData += '<td style="width:10%; padding:0px 5px 0px 5px; text-align:center;">';
                        tableData += result.data[i].po_number + '</td>';

                        tableData += '<td style="width:5%; padding:0px 5px 0px 5px; text-align:center;">';
                        tableData += result.data[i].item_line + '</td>';

                        tableData += '<td style="width:10%; padding:0px 5px 0px 5px; text-align:center;">';
                        tableData += result.data[i].issue_date + '</td>';

                        tableData += '<td style="width:10%; padding:0px 5px 0px 5px; text-align:center;">';
                        tableData += result.data[i].eta_date + '</td>';

                        tableData += '<td style="width:5%; padding:0px 5px 0px 5px; text-align:right;">';
                        tableData += result.data[i].quantity + '</td>';

                        tableData += '<td style="width:5%; padding:18px 5px 0px 5px; text-align:center;">';
                        tableData += '<label>';
                        tableData += '<input type="checkbox" class="minimal" ';
                        tableData += 'id="check_' + result.data[i].item_line + '">';
                        tableData += '</label><br>'
                        tableData += '</td>';

                        tableData += '<td style="width:15%; padding:0px 5px 0px 5px; text-align:center;">';
                        tableData += '<input type="text" class="form-control" style="font-size: 12px;" ';
                        tableData += 'id="note_' + result.data[i].item_line + '" placeholder="Reason ...">';
                        tableData += '</td>';
                    ...
                    openErrorGritter('Error!',
                        "There is a system error. Please contact the buyer for assistance");
