		...
		<div class="col-md-6" style="padding-right: 0; padding-left: 0">
			<table class="table table-bordered" style="width: 100%; margin-bottom: 2px;" border="1">
				<tbody>
					...
					<tr>
						<td style="background-color: #fca311; color: #14213d; text-align: center; font-size:15px;" id="date">{{$outgoing[0]->check_date}}</td>
			...
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					...
					<tr>
						<td colspan="2">
							<input type="text" class="pull-right" name="material_number" style="height: 50px;font-size: 2vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="material_number" placeholder="Material Number" value="{{$outgoing[0]->material_number}}" readonly>
						</td>
					</tr>
					<tr>
					...
					<tr>
						<td id="material_description" style="background-color: #fca311; text-align: center; color: #14213d; font-size: 20px;">{{$outgoing[0]->material_description}}
						</td>
			...
			<table class="table table-bordered" style="width: 100%; margin-bottom: 5px;border: 0">
				<tbody>
					...
					<tr>
						...
						<td>
							<input type="number" class="pull-right" name="qty_check" style="height: 50px;font-size: 2vw;width: 100%;text-align: center;vertical-align: middle;color: #14213d" id="qty_check" placeholder="Quantity Check" onchange="checkQty(this.value);" value="{{$outgoing[0]->qty_check}}">
						</td>
					</tr>
				</tbody>
			</table>
				...
				<table class="table table-bordered" style="width: 100%; margin-bottom: 2px;" border="1" id="tableNgList">
					...
					<tbody id="bodyTableNgList">
						...
						<tr <?php echo $color ?>>
							...
							<td><input type="number" class="numpad" style="font-weight: bold; font-size: 45px; background-color: rgb(100,100,100); color: yellow;width: 100%;text-align: center;" id="count{{$nomor+1}}" value="0" onchange="checkQtyNg(this.value,this.id)" placeholder="0"></td>
						</tr>
						<?php $no+=1; ?>
						@endforeach
					</tbody>
...
<script>
	...
	function confirmNgLog() {
		...
		if ($('#material_description').text() == "-" || $('#qty_check').val() == "") {
			...
			$.post('{{ url("index/outgoing/cpp/confirm/recheck") }}', data, function(result, status, xhr){
				if(result.status){
					cancelAll();
					// updateKensaCode();
					...
					window.location.replace('{{url("index/outgoing/cpp")}}');
				}else{
					openErrorGritter('Error!',result.message);
					$('#loading').hide();
					$('#btn_confirm').removeAttr('disabled');
