...
@section('content')
    <section id="vfi-container">
        ...
        <div class="row" style="padding-top: 20px; padding-left: 20px; padding-right: 20px; margin-bottom: 10px;">
            <a style="margin-bottom: 10px;" class="btn btn-danger" href="{{ url('') }}"><i
                    class="fa fa-arrow-left"></i>Kembali</a>
            <a class="btn btn-success" href="{{ url('index/driver/attendance') }}">Input Kehadiran</a>
        </div>
        <div class="row" id="divDriver"
            style="padding-bottom: 20px; padding-left: 20px; padding-right: 20px; overflow-y: scroll;">
            ...
            $.get('{{ url('fetch/driver/attendance/report') }}', function(result, status, xhr) {
                if (result.status) {
                    ...
                    for (var i = 0; i < result.attendance.length; i++) {
                        attendance +=
                            '<div class="col-lg-12 col-md-12 col-sm-12" style="padding-left: 0px;padding-right: 0px;margin-bottom: 10px;">';
                        attendance += '<table style="border: 1px solid black;width: 100%">';
                        attendance += '<tr>';
                        attendance +=
                            '<td style="border: 1px solid black;padding-left:4px;font-weight:bold;" colspan="2"><span class="pull-left">' +
                            result.attendance[i].date + '</span></td>';
                        attendance += '</tr>';
                        attendance += '<tr>';
                        ...
                        if(result.attendance[i].start_asli >= '{{date("Y-m-d")}} 04:00:00' && result.attendance[i].start_asli <= '{{date("Y-m-d")}} 07:00:00' && '{{date("Y-m-d H:i:s")}}' <= '{{date("Y-m-d")}} 17:00:00'){
                            ...
                            attendance += '<span class="pull-right" style="padding-right:2px;color:cornflowerblue; cursor:pointer; float: right;" onclick="showLoc(\'' +
                            i + '\',\'' + result.attendance.length + '\',\'' + starts +
                            '\')"><i class="fas fa-map-marker-alt"></i></span>';
                        }
                        attendance += '</td>';
                        if (result.attendance[i].startss == result.attendance[i].endss) {
                            attendance +=
                                '<td style="border: 1px solid black;padding-left:4px;font-weight: bold;font-size: 20px;color: #eb0000">--:--</td>';
                        } else {
                            attendance +=
                            '<td style="border: 1px solid black;padding-left:4px;font-weight: bold;font-size: 20px;color: red">' +
                            result.attendance[i].endss;
                            if(result.attendance[i].end_asli >= '{{date("Y-m-d")}} 15:40:00' && result.attendance[i].end_asli <= '{{date("Y-m-d")}} 17:30:00' && '{{date("Y-m-d H:i:s")}}' >= '{{date("Y-m-d")}} 15:40:00'){
                                var url_penumpang = '{{ url("index/passenger/attendance") }}';
                                attendance += '<a class="pull-right" style="color: coral; float: right;" href="'+url_penumpang+'"><i class="fa fa-users"></i></a>';
                            }else{
                                attendance += '<span class="pull-right" style="padding-right:2px;color:cornflowerblue; cursor:pointer; float: right;" onclick="showLoc(\'' +
                                i + '\',\'' + result.attendance.length + '\',\'' + ends +
                                '\')"><i class="fas fa-map-marker-alt"></i></span>';
                            }
                            attendance += '</td>';
                        }
                        ...
                        attendance +=
                            '<td style="border: 1px solid black;padding-left:4px;font-weight:bold;text-align:center;" colspan="2">';
                        attendance +=
                            '<div class="mapouter"><div class="gmap_canvas"><iframe width="300" height="200" id="gmap_canvas" src="https://maps.google.com/maps?q=' +
                            result.attendance[i].latitude_start + ',' + result.attendance[i].longitude_start +
                        ...
                        if (result.attendance[i].startss != result.attendance[i].endss) {
                            ...
                            attendance +=
                                '<div class="mapouter"><div class="gmap_canvas"><iframe width="300" height="200" id="gmap_canvas" src="https://maps.google.com/maps?q=' +
                                result.attendance[i].latitude_end + ',' + result.attendance[i].longitude_end +
                                '&t=&z=10&ie=UTF8&iwloc=&output=embed" frameborder="0" scrolling="no" marginheight="0" marginwidth="0"></iframe><a href="https://2yu.co">2yu</a><br><style>.mapouter{position:relative;text-align:right;height:200px;width:300px;}</style><a href="https://embedgooglemap.2yu.co">html embed google map</a><style>.gmap_canvas {overflow:hidden;background:none!important;height:200px;width:300px;}</style></div></div>';
                            attendance += '</td>';
                            attendance += '</tr>';
                        }
