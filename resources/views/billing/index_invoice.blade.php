...
<div class="modal fade" id="modalRejectInvoice" tabindex="-1" role="dialog" aria-labelledby="exampleModalLabel" aria-hidden="true">
        <div class="modal-dialog">
            <div class="modal-content">
                <div class="modal-header">
                    <button type="button" class="close" onclick="modalrejectinvoicehide()">&times;</button>
                    <h4 class="modal-title" id="myModalLabel"></h4>
                </div>
                <div class="modal-body">

                    <label for="reason_reject" class="control-label">Masukkan Alasan ingin Request Reject Invoice ini : <span class="text-red">*</span></label>

                    <textarea name="reason_reject" id="reason_reject" cols="30" rows="10" class="form-control"></textarea>

                    <br>
                    Apakah anda yakin ingin melakukan request ini ?
                </div>
                <div class="modal-footer">
                    <button type="button" class="btn btn-default" onclick="modalrejecthide()">Cancel</button>
                    <a id="a" name="modalButtonInvoice" type="button"  onclick="request_reject(this.id)" class="btn btn-danger" style="color:white">Request Reject</a>
                </div>
            </div>
        </div>
...
<script type="text/javascript">
                ...
                for (var i = 0; i < result.invoice.length; i++) {
                    ...
                    if ("{{ Auth::user()->role_code == 'E - Billing'}}") {
                        ...
                        if (result.invoice[i].status == "Open") {
                            bodyTable += '<td><span class="label label-warning">Check By Purchasing</span> <a href="javascript:void(0)" class="btn btn-xs btn-danger" onClick="reject('+result.invoice[i].id+')" style="margin-right:5px;color:white" title="Request Reject Invoice"><i class="fa fa-rotate-left"></i> Request Reject</a></td>';
                        }
    ...
    function reject(id) {
        $('#modalRejectInvoice').modal('show');
        $('[name=modalButtonInvoice]').attr("id","reject_invoice_"+id);
    }

    function modalrejectinvoicehide(){
        $('#modalRejectInvoice').modal('hide');
    }
    ...
    function request_reject(id){
        ...
        $.post('{{ url("request/reject/invoice") }}', data, function(result, status, xhr){
            if (result.status == true) {
                openSuccessGritter("Success","Request Berhasil Dikirim");
                $("#loading").hide();
                modalrejectinvoicehide();
            }
            else{
                openErrorGritter("Success","Data Gagal Dihapus");
            }
