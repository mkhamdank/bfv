...
use App\Models\AccInvoiceVendor;
use App\Models\AccInvoicePaymentTerm;
use App\Models\AccPaymentRequest;
use App\Models\AccPaymentRequestDetail;
use App\Models\AccSupplier;
use App\Models\AccJurnal;
use App\Models\AccJurnalDetail;
use App\Models\AccJurnalInvoice;
...
class AccountingController extends Controller
{
    ...
    public function uploadInvoicePost(request $request)
    {
        try {
            ...
            $cek_tagihan = AccInvoiceVendor::where('tagihan', $request->get('tagihan'))->get();

            if (count($cek_tagihan) > 0) {
                return back()->with('error', 'Invoice Number Already Exist')
                    ->with('page', 'Upload Invoice');
            }
    ...
    public function requestRejectInvoice(Request $request)
    {
        try {
            $invoice = AccInvoiceVendor::find($request->get('id'));
            $invoice->note_vendor = $request->get('note');
            $invoice->save();

            Mail::to(['shega.erik.wicaksono@music.yamaha.com'])->bcc(['rio.irvansyah@music.yamaha.com'])->send(new SendEmail($invoice, 'request_reject'));
    ...
    public function createPaymentRequest(Request $request)
    {
        try {
            ...
            $pdf->loadView(
                'billing.report_payment_request',
                array(
    ...
    public function verifikasi_payment_request($id)
    {
        ...
        return view(
            'billing.verifikasi_payment_request',
            array(
                'payment' => $payment,
                'payment_detail' => $payment_detail,
                'file_path' => $file_path,
    ...
    {
        return view(
            'billing.monitoring_payment_request',
            array(
   ...
   ");
    ...
    public function indexVendorRegistration()
    {
        ...
        if (Auth::user()->role_code == "MIS" || Auth::user()->role_code == "E - Purchasing") {
            return view(
                'billing.purchasing.vendor_registration',
                array(
                    'title' => $title,
                    'title_jp' => $title_jp,
                    'vendor' => $vendor
    ...
    public function deleteVendorRegistration(Request $request)
    {
        try {
            ...
            $vendor->remark = $request->get('note');
            $vendor->save();
    ...
    public function indexCostCenter()
    {
        ...
        if (Auth::user()->role_code == "MIS" || Auth::user()->role_code == "E - Accounting") {
            return view(
                'billing.master.index_cost_center',
                array(
                    'title' => $title,
                    'title_jp' => $title_jp
                )
    ...
    public function indexGLAccount()
    {
        ...
        if (Auth::user()->role_code == "MIS" || Auth::user()->role_code == "E - Accounting") {
            return view(
                'billing.master.index_gl_account',
                array(
                    'title' => $title,
                    'title_jp' => $title_jp
                )
    ...
    {
        $title = 'Fixed Asset Check';
        $title_jp = '';

        $period = FixedAssetAudit::select(db::raw('DATE_FORMAT(period,"%Y %M") as period'))
            ->where('category', 'Vendor')
            ->groupby(db::raw('DATE_FORMAT(period,"%Y %M")'))
            ->orderBy('period', 'DESC')->get();

        $permiss = db::table('fixed_asset_permissions')->select('permission')->where('user_id', Auth::user()->id)->first();
    ...
    public function inputAssetCheckTemp(Request $request)
    {
        try {
            if ($request->hasFile('fileData')) {
                $tujuan_upload = 'data_file/fixed_asset/check_picture/';
                ...
                $additionalColumns = [
                    ...
                    'label_condition' => $request->get('label_condition'),
                    'usable_condition' => $request->get('usable_condition'),
                    'map_condition' => $request->get('map_condition'),
                ...
                if ($request->get('category') == 'check1') {
                    DB::table('fixed_asset_checks')
                        ->where('sap_number', $request->get('asset_id'))->where('period', $request->get('period'))
                        ->update(array_merge($additionalColumns, [
                            'check_one_by' => Auth::user()->username . '/' . Auth::user()->name,
                            'check_one_at' => date('Y-m-d H:i:s'),
                            'remark' => 'temporary save 1',
                        ]));
                ...
                if ($request->get('category') == 'check1') {
                    DB::table('fixed_asset_checks')
                        ->where('sap_number', $request->get('asset_id'))->where('period', $request->get('period'))
                        ->update(array_merge($additionalColumns2, [
                            'check_one_by' => Auth::user()->username . '/' . Auth::user()->name,
                            'check_one_at' => date('Y-m-d H:i:s'),
                            'remark' => 'temporary save 1',
                        ]));
    ...
    public function inputAssetCheck(Request $request)
    {
        try {
            if ($request->hasFile('fileData')) {
                ...
                //     SET ympimis.fixed_asset_checks.result_images = '" . $filename . "',
                //     ympimis.fixed_asset_checks.note = '" . $request->get('note') . "',
                //     ympimis.fixed_asset_checks.availability = '" . $request->get('availability') . "',
                //     ympimis.fixed_asset_checks.asset_condition = '" . $request->get('asset_condition') . "',
                //     ympimis.fixed_asset_checks.label_condition = '" . $request->get('label_condition') . "',
                //     ympimis.fixed_asset_checks.usable_condition = '" . $request->get('usable_condition') . "',
                //     ympimis.fixed_asset_checks.map_condition = '" . $request->get('map_condition') . "',
                //     ympimis.fixed_asset_checks.asset_image_condition = '" . $request->get('image_condition') . "',
                //     ympimis.fixed_asset_checks.status = 'Check 1',
                //     ympimis.fixed_asset_checks.check_one_by = '" . Auth::user()->username . '/' . Auth::user()->name . "',
                //     ympimis.fixed_asset_checks.check_one_at = '" . date('Y-m-d H:i:s') . "'
                //     WHERE ympimis.fixed_asset_checks.period = '" . $request->get('period') . "' AND ympimis.fixed_asset_checks.sap_number = '" . $request->get('asset_id') . "'");
                ...
                //     ympimis.fixed_asset_checks.map_condition = '" . $request->get('map_condition') . "',
                //     ympimis.fixed_asset_checks.asset_image_condition = '" . $request->get('image_condition') . "',
                //     ympimis.fixed_asset_checks.status = 'Check 2',
                //     ympimis.fixed_asset_checks.check_two_by = '" . Auth::user()->username . '/' . Auth::user()->name . "',
                //     ympimis.fixed_asset_checks.check_two_at = '" . date('Y-m-d H:i:s') . "'
                //     WHERE ympimis.fixed_asset_checks.period = '" . $request->get('period') . "' AND ympimis.fixed_asset_checks.sap_number = '" . $request->get('asset_id') . "'");
                // }
                ...
                if ($request->get('index') == $request->get('counter')) {
                    ...
                    if ($request->get('category') == 'check1') {
                        $update_asset = db::table('fixed_asset_checks')->where('period', $request->get('period'))
                ...
                //     $update_mirai = db::select("UPDATE ympimis.fixed_asset_checks
                //     SET ympimis.fixed_asset_checks.note = '" . $request->get('note') . "',
                //     ympimis.fixed_asset_checks.availability = '" . $request->get('availability') . "',
                //     ympimis.fixed_asset_checks.asset_condition = '" . $request->get('asset_condition') . "',
                //     ympimis.fixed_asset_checks.label_condition = '" . $request->get('label_condition') . "',
                //     ympimis.fixed_asset_checks.usable_condition = '" . $request->get('usable_condition') . "',
                ...
                if ($request->get('index') == $request->get('counter')) {
                    ...
                    if ($request->get('category') == 'check1') {
                        db::table('fixed_asset_checks')->where('period', $request->get('period'))
                            ->where('location', '=', $request->get('location'))
                            ->update([
                                'status' => 'Check 1',
                                'remark' => null
    ...
    public function indexAssetAuditListAuditor()
    {
        ...
        return view(
            'fixed_asset.form.auditor_audit_list',
            array(
                'title' => $title,
                'title_jp' => $title_jp,
    ...
    {
        $asset_qty = db::table('fixed_asset_checks')->where('category', '=', $request->get('category'))
        ...
        if (count($asset_qty) > 0) {
            $response = array(
                'status' => false,
                'message' => 'Semua Asset dalam lokasi harus sudah dicek'
            );
            return Response::json($response);
        }

        $asset_stat = db::table('fixed_asset_checks')->where('category', '=', $request->get('category'))
            ->where('location', '=', $request->get('location'))
            ->where('period', '=', $request->get('period'))
        ...
        $asset_check = db::table('fixed_asset_checks')->where('category', '=', $request->get('category'))
            ->where('status', '=', 'Check 2')
            ->where('location', '=', $request->get('location'))
            ->where('period', '=', $request->get('period'))
            ->select('period', 'location', db::raw('count(sap_number) as total_asset'), 'audit_type')
            ->groupBy('period', 'location', 'audit_type')
            ->get();

        db::table('fixed_asset_checks')->where('category', '=', $request->get('category'))
            ->where('status', '=', 'Check 2')
            ->where('location', '=', $request->get('location'))
            ->where('period', '=', $request->get('period'))
            ->update([
        ...
        // if ($asset_check[0]->audit_type == 'Normal') {
    ...
    public function pdfFixedAsset($location, $period)
    {
        ...
        $pdf->loadView(
            'fixed_asset.report_file.audit_report',
            array(
                'check_data' => $check_data,
            )
        );

        // $pdf->save(public_path() . "/payment_list/Payment ".$request->input('supplier_name'). " ".date('d-M-y', strtotime($request->input('payment_date'))).".pdf");
        return $pdf->stream("Audit Fixed Asset " . $period . "_" . $location . ".pdf");
    }

    function indexFixedAssetAudit()
    {
        $title = 'Fixed Asset Audit';
        $title_jp = '';

        $perm = db::table("fixed_asset_permissions")->select("user_id", "username")->where("permission", 'Auditor')->get();
        ...
        if ($stts_join) {
            ...
            return redirect()->back()->with('message', 'You Dont Have Authorization');
        }
    ...
    function fetchAssetMasterAuditList(Request $request)
    {
        ...
        (select period, asset_section, location, count(id) as jml_asset, `status`, appr_manager_at from fixed_asset_checks
            where DATE_FORMAT(period, "%Y %M") = "' . $period . '"
            ...
            left join users on users.username = audit.pic');
    ...
    public function indexAssetAudit($cek_num, $location, $period)
    {
        $title = "Form Check Fixed Asset";
        $title_jp = "??";
    ...
    public function inputAssetAuditTemp(Request $request)
    {
        try {
            if ($request->hasFile('fileData')) {
                $tujuan_upload = 'data_file/fixed_asset/audit_picture/';
                $file = $request->file('fileData');
                $filename = $request->get('period') . '_' . $request->get('category') . '_' . $request->input('asset_id') . '.' . $request->input('extension');
                $file->move($tujuan_upload, $filename);

                DB::table('fixed_asset_audits')
                    ->where('sap_number', $request->get('asset_id'))->where('period', $request->get('period'))
                    ->update([
                        'availability' => $request->get('availability'),
                        'asset_condition' => $request->get('asset_condition'),
                ...
                $response = array(
                    'status' => false,
                    'message' => 'Upload Photo on Asset "' . $request->input('asset_name') . '"'
                );
                return Response::json($response);
            }
        } catch (\Exception $e) {
    ...
    function saveAssetAudit(Request $request)
    {
        try {
            DB::table('fixed_asset_audits')
    ...
    public function generateAssetPDF(Request $request)
    {
        if ($request->get("category") == "Check") {
            $datas = db::table('fixed_asset_checks')
                ->where('period', $request->get('period'))
                ->where('location', $request->get('location'))
                ->select('id', 'period', 'category', 'location', 'sap_number', 'asset_name', 'asset_section', 'asset_images', 'pic', 'result_images', 'note', 'availability', 'asset_condition', 'label_condition', 'usable_condition', 'map_condition', 'asset_image_condition', 'status', 'check_one_by', 'check_one_at', 'check_two_by', 'check_two_at', 'appr_chief_by', 'appr_chief_at', 'appr_manager_by', 'appr_manager_at', 'appr_status', 'remark', 'audit_type')
                ->get();
            ...
            $pdf->save(public_path() . "/data_file/fixed_asset/report/Check " . $request->get('period') . "_" . $request->get('location') . ".pdf");
        } else {
            $datas = db::table('fixed_asset_audits')
                // ->leftJoin('fixed_asset_checks', )
                ->where('period', $request->get('period'))
                ->where('location', $request->get('location'))
                ->select('id', 'period', 'category', 'location', 'sap_number', 'asset_name', 'asset_section', 'asset_images', 'pic', 'result_images', 'note', 'availability', 'asset_condition', 'label_condition', 'usable_condition', 'map_condition', 'asset_image_condition', 'status', 'check_one_by', 'check_one_at', 'check_two_by', 'check_two_at', 'appr_chief_by', 'appr_chief_at', 'appr_manager_by', 'appr_manager_at', 'appr_status', 'remark', 'audit_type')
                ->get();
            ...
            $pdf->save(public_path() . "/data_file/fixed_asset/report/Audit " . $request->get('period') . "_" . $request->get('location') . ".pdf");
        }

    }

    function checkFixedAssetReport(Request $req)
    {
        $file = File::exists(public_path('data_file/fixed_asset/report/' . $req->get('category') . ' ' . $req->get('period') . "_" . $req->get('location') . ".pdf"));

        if ($file) {
            $response = array(
                'status' => true,
                'message' => '/data_file/fixed_asset/report/' . $req->get('category') . ' ' . $req->get('period') . "_" . $req->get('location') . ".pdf"
            );
            return Response::json($response);
        } else {
            $response = array(
                'status' => false,
                'message' => 'File report tidak ditemukan, Mohon untuk generate terlebih dahulu'
            );
            return Response::json($response);
        }
    }

    function indexFixedAssetReport()
    {
        $title = "Monitoring Check & Audit";
        $title_jp = "??";

        $period = FixedAssetAudit::select('period')->groupby('period')->get();

        return view(
            'fixed_asset.monitoring_audit',
            array(
                'title' => $title,
                'title_jp' => $title_jp,
                'period' => $period,
    ...
    function fetchFixedAssetReport(Request $request)
    {
        try {
            $period_data = db::table('fixed_asset_checks')->select('period')->orderBy('period', 'DESC')->first();

            $period = $period_data->period;
            $period_name = date('M Y', strtotime($period_data->period));
            if ($request->get('period')) {
                $period = $request->get('period');
            ...
            (
                ...
                'Check 1',
            IF
            ( remark = 'temporary save 2', 'Check 2', `status` )) AS stat,
            IF
            (
                appr_status IS NOT NULL 
                AND appr_manager_at IS NULL,
                'Waiting Approval',
            IF
            ( appr_status IS NOT NULL AND appr_manager_at IS NOT NULL, 'Fully Approve', '' )) appr_stat ,
            availability, asset_condition, label_condition, usable_condition, map_condition, asset_image_condition, note
            FROM
            fixed_asset_checks 
            WHERE
            period = '" . $period . "'
            ...
            $checked_data = db::select("SELECT location, real_stat, COUNT(asset_name) as total_asset FROM
            (SELECT sap_number, asset_name, location, IF(appr_stat != '', appr_stat, stat) real_stat FROM
            (SELECT
            sap_number,
            asset_name,
            location,
            ...
            WHERE
            period = '" . $period . "'
            order by stat ) mstr ) mstr2
            GROUP by location, real_stat");

            $audited_data = db::select("SELECT location, SUM(IF(stat = 'Audited',1,0)) as audited, SUM(IF(stat = 'Finding',1,0)) as finding, COUNT(sap_number) as total_asset FROM (SELECT period, category, location, sap_number, asset_name, asset_section, 
            IF(`remark` = 'audited', IF(availability <> std_availability OR asset_condition <> std_cond OR label_condition <> label OR usable_condition <> usable OR map_condition <> map OR asset_image_condition <> image, 'Finding', 'Audited'),'') as stat
            FROM
            ...
            $response = array(
                ...
                'details_check_data' => $details_check_data,
                'audited_data' => $audited_data,
                'details_audited_data' => $details_audited_data,
                'period' => $period_name
            );
            return Response::json($response);
        } catch (\Throwable $th) {
